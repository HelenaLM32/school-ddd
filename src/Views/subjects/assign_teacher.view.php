    ...
    <div class="col-md-8 col-lg-6">
      ...
      <div class="card shadow-sm border-0 rounded-3 mb-4">
        <div class="card-header bg-primary text-white">
          <h3 class="mb-0">Assignar Professor a Assignatura</h3>
        </div>
        <div class="card-body">
          <form action="/subject/assign-teacher/store" method="POST">
            <div class="mb-3">
              <label class="form-label" for="teacher_id">Professor</label>
            ...
            <button type="submit" class="btn btn-primary w-100">Assignar</button>
          </form>
        </div>
      ...
      <!-- Estat actual de les assignatures -->
      <?php if (!empty($subjects)): ?>
      <div class="card shadow-sm border-0 rounded-3">
        <div class="card-header bg-secondary text-white">
          <h5 class="mb-0">Assignatures i professors actuals</h5>
        </div>
        <div class="card-body p-0">
          <table class="table table-striped table-hover mb-0">
            <thead class="table-light">
              <tr><th>Assignatura</th><th>Professor assignat</th></tr>
            </thead>
            <tbody>
              <?php foreach ($subjects as $sub):
                ...
                if ($sub->teacherId()) {
                  foreach ($teachers ?? [] as $t) {
                    if ($t->id()->value() === $sub->teacherId()->value()) {
